...
export default function FleetManagement() {
  ...
  async function fetchVehicles() {
    try {
      const response = await axios.get("http://localhost:8989/api/vehicles", {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8989/api/vehicles/${editingVehicle._id}`, {
        ...editingVehicle,
  ...
  const handleSaveDriverLink = async () => {
    ...
    try {
      const response = await axios.put(`http://localhost:8989/api/vehicles/${linkingDriver._id}/driver`, {
  ...
  const handleSuspend = async (vehicleId) => {
    try {
      const response = await axios.put(`http://localhost:8989/api/vehicles/${vehicleId}/status`, {
        status: "Inactive",
      });
      setCurrentVehicles((prev) =>
        prev.map((vehicle) =>
  ...
  const handleAddVehicle = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:8989/api/vehicles", {
        ...newVehicle,
